...
namespace RunWalkProV2
{
    public partial class PivMain : PhoneApplicationPage
    {
        ...
        public PivMain()
        {
            ...
            // check screen lock first time
            if (!IsolatedStorageSettings.ApplicationSettings.Contains("PreventLock"))
            {
                IsolatedStorageSettings.ApplicationSettings.Add("PreventLock", true);
            }
            else
            {
                if((bool)IsolatedStorageSettings.ApplicationSettings["PreventLock"] == true)
                {
            ...
            // get for current workout
            checkForCurrentWorkout();

        }

        private void checkForCurrentWorkout()
        {
            if(_params.CurrentDistance > 0)
            {
                SessionWorkout.Distance = _params.CurrentDistance;
            }

            if(_params.TotalCurrentTimeInSeconds > 0)
            {
                SessionWorkout.BaseSeconds = _params.TotalCurrentTimeInSeconds;
            }

            setUpCurrentWorkoutOnStart();
        }
        ...
        private void getUserSetting()
        {
            ...
            UserSettings.RunWalkPROUserName = userset.RunWalkPROUserName;
            UserSettings.RunWalkPROToken = userset.RunWalkPROToken;
        ...
        private void Watcher_PositionChanged(object sender, GeoPositionChangedEventArgs<GeoCoordinate> e)
        {
            ...
            checkDistanceGoal();
            setCurrentDistance(SessionWorkout.Distance);

        }

        private void setCurrentDistance(double p)
        {
            _params.CurrentDistance = p;
        }
        ...
        private void stop(object sender, EventArgs e)
        {
            ...
            _params.TotalCurrentTimeInSeconds = 0;
            _params.TotalCurrentCalories = 0;
            _params.CurrentDistance = 0;
        ...
        private void OnTimerTick(object sender, EventArgs e)
        {
            if (SessionWorkout.IntervalStartTime == DateTime.MinValue)
            {
                ...
                if (SessionWorkout.BaseSeconds > 0)
                {
                    SessionWorkout.IntervalStartTime = SessionWorkout.IntervalStartTime.Subtract(TimeSpan.FromSeconds(SessionWorkout.BaseSeconds));
                    SessionWorkout.StartTime = SessionWorkout.StartTime.Subtract(TimeSpan.FromSeconds(SessionWorkout.BaseSeconds));
                }
            }
            ...
            checkTimeGoal(currentValue.TotalSeconds);

            setCurrentTime(currentValue.TotalSeconds);
            
        }

        private void setCurrentTime(double p)
        {
            _params.TotalCurrentTimeInSeconds = p;
        }
        ...
        private void txtbConnectOnline_Tap(object sender, System.Windows.Input.GestureEventArgs e)
        {
            NavigationService.Navigate(new Uri("/RWPLogin.xaml", UriKind.Relative));
        }

        private void txtbNewWO_Tap(object sender, System.Windows.Input.GestureEventArgs e)
        {
            stop(null, null);
            pivMainP.SelectedIndex = 1;
        }
        
        private void txtbCurrentWO_Tap(object sender, System.Windows.Input.GestureEventArgs e)
        {
            pivMainP.SelectedIndex = 1;
        }

        private void imgCurrWO_Tap(object sender, System.Windows.Input.GestureEventArgs e)
        {
            pivMainP.SelectedIndex = 1;
        }

        private void imgNewWO_Tap(object sender, System.Windows.Input.GestureEventArgs e)
        {
            stop(null, null);
            pivMainP.SelectedIndex = 1;
        }

        private void imgWWW_Tap(object sender, System.Windows.Input.GestureEventArgs e)
        {
            NavigationService.Navigate(new Uri("/RWPLogin.xaml", UriKind.Relative));
        }

        private void setUpCurrentWorkoutOnStart()
        {
            if (UserSettings.UnitOfMeasure == "Miles")
            {
                TimeSpan currenttime = TimeSpan.FromSeconds(SessionWorkout.BaseSeconds);
                TimeSpan pace = CalculatePPD(currenttime, SessionWorkout.Distance);
                txtBlkPaceMain.Text = pace.ToString(@"mm\:ss");
                SetPaceColor(pace);
                txtBlkDistanceMain.Text = string.Format("{0:f2}", SessionWorkout.Distance);
                txtBlkCaloriesMain.Text = string.Format("{0:f0}", SessionWorkout.Distance * UserSettings.CaloriesPerUnit);
                txtPacePer.Text = "Mins./Mile";
            }
            else
            {
            ...
            TimeSpan currentValue = TimeSpan.FromSeconds(SessionWorkout.BaseSeconds);
